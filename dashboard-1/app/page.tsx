...
export default function MaritimeDashboard() {
  ...
  return (
    <div className="min-h-screen text-white font-sans relative flex flex-col">
      ...
      {/* Header */}
      <header className="flex flex-wrap items-center justify-between px-6 sm:px-8 py-5 border-b border-[#1e3a5f]/50 gap-4">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-6xl sm:text-7xl font-semibold tracking-wide">
        ...
        <div className="flex items-center gap-3 sm:gap-4 flex-wrap">
          <div className="flex items-center bg-[#0d2137] rounded-lg overflow-hidden border border-[#1e3a5f]">
            <span className="text-gray-400 text-base px-3 hidden sm:block">search:</span>
            <input
              type="text"
              placeholder="Vessel ID..."
              value={vesselId}
              onChange={(e) => setVesselId(e.target.value)}
              className="bg-[#142d4a] px-3 py-2 text-base text-cyan-300 placeholder-cyan-700 focus:outline-none w-32 sm:w-40"
            />
            <button
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <div className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-[#020817] ${serverStatus === "Backend Connected" ? "bg-green-400" : "bg-red-500"}`} />
            </div>
            <div className="text-right hidden sm:block">
              <p className="text-base font-medium">{boatId}</p>
              <p className={`text-sm ${serverStatus === "Backend Connected" ? "text-green-400" : "text-red-400"}`}>{serverStatus}</p>
            </div>
          </div>
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all border ${
              demoMode
                ? "bg-purple-700/80 border-purple-400 text-white shadow-[0_0_16px_rgba(147,51,234,0.5)]"
                : "bg-[#0d2137] border-[#1e3a5f] text-gray-300 hover:border-purple-500 hover:text-purple-300"
            }`}
          >
            <span className={`w-2.5 h-2.5 rounded-full ${demoMode ? "bg-purple-300 animate-pulse" : "bg-gray-600"}`} />
      ...
      <div className="flex flex-wrap items-center gap-6 px-6 sm:px-8 py-4 border-b border-[#1e3a5f]/40"
        style={{ background: 'linear-gradient(135deg, rgba(7,21,37,0.97) 0%, rgba(10,22,40,0.95) 100%)' }}
      >
        <span className="text-gray-500 text-xl font-semibold uppercase tracking-widest">Hardware Indicators</span>
        ...
        <div className="flex items-center gap-3">
          <div className="relative flex items-center justify-center w-10 h-10">
            ...
            <div className={`w-7 h-7 rounded-full border-2 transition-all duration-300 ${
              zone === "DANGER"
                ? "bg-red-500 border-red-300 shadow-[0_0_18px_6px_rgba(239,68,68,0.8)]"
                : zone === "WARNING"
                  ? "bg-yellow-400 border-yellow-200 shadow-[0_0_18px_6px_rgba(250,204,21,0.7)] animate-pulse"
                  : "bg-gray-700 border-gray-600"
            }`} />
          </div>
          <div>
            <p className="text-lg font-bold text-gray-400 uppercase tracking-wider leading-none">LED</p>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-lg font-bold text-gray-400 uppercase tracking-wider leading-none">BUZZER</p>
            <p className={`text-xl font-bold leading-tight ${
        ...
        {demoMode && (
          <div className="ml-auto flex items-center gap-2 px-4 py-2 rounded-full border border-purple-500/50 bg-purple-900/30">
            <span className="w-2.5 h-2.5 rounded-full bg-purple-400 animate-pulse" />
            <span className="text-purple-300 text-xl font-bold uppercase tracking-wider">Demo Mode Active</span>
          </div>
        )}
      </div>
      ...
      {/* Bottom Panels: Distance Chart + Alert Log */}
      <div className="flex flex-col lg:flex-row px-6 sm:px-10 gap-6 mb-8">
        <div className="flex-1 rounded-2xl p-10 border border-[#1e3a5f]/50" style={{ background: 'linear-gradient(135deg, rgba(13,33,55,0.95) 0%, rgba(10,22,40,0.9) 100%)' }}>
          <h2 className="text-4xl font-semibold text-cyan-400 mb-6 uppercase tracking-wider">Distance to Boundary (km)</h2>
          {history.length > 0 ? (
            <ResponsiveContainer width="100%" height={480}>
              <LineChart data={[...history].reverse().slice(-30).map(d => ({
                time: new Date(d.timestamp).toLocaleTimeString('en', { hour: '2-digit', minute: '2-digit' }),
                distance: Number((d.distance ?? 0).toFixed(1)),
              }))}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(30,58,95,0.5)" />
                <XAxis dataKey="time" stroke="#475569" fontSize={20} tick={{ fill: '#94a3b8' }} interval="preserveStartEnd" />
                <YAxis stroke="#475569" fontSize={20} tick={{ fill: '#94a3b8' }} />
                <Tooltip
                  contentStyle={{ background: '#0a1628', border: '1px solid rgba(6,182,212,0.4)', borderRadius: '8px', fontSize: '22px' }}
                  labelStyle={{ color: '#06b6d4' }}
                  itemStyle={{ color: '#38bdf8' }}
                />
                <Line type="monotone" dataKey="distance" stroke="#06b6d4" strokeWidth={4} dot={false} activeDot={{ r: 7, fill: '#06b6d4' }} />
              </LineChart>
            </ResponsiveContainer>
          ) : (
            <div className="h-[480px] flex items-center justify-center text-gray-500 text-2xl">No history — waiting for vessel data</div>
          )}
        </div>
        ...
        <div className="lg:w-[600px] rounded-2xl p-10 border border-[#1e3a5f]/50" style={{ background: 'linear-gradient(135deg, rgba(13,33,55,0.95) 0%, rgba(10,22,40,0.9) 100%)' }}>
          ...
          <div className="overflow-y-auto max-h-[480px] space-y-4 pr-2">
            {alerts.length === 0 ? (
              <p className="text-gray-500 text-2xl text-center py-8">No zone changes recorded</p>
            ) : (
              alerts.map((a, i) => (
